...
class MoEGate(nn.Module):
    def __init__(self, config, idx):
        ...
        self.variance_loss_target = getattr(config, 'variance_loss_weight', 0.01)
        ...
        # tensorboard logging
        self.layer_idx = idx
        aaa=f'{self.alpha}'.split('.')[0] + f'{self.alpha}'.split('.')[1] if len(f'{self.alpha}'.split('.')) > 1 else f'{self.alpha}'
        vvv=f'{self.variance_loss_weight}'.split('.')[0] + f'{self.variance_loss_weight}'.split('.')[1] if len(f'{self.variance_loss_weight}'.split('.')) > 1 else f'{self.variance_loss_weight}'
        ooo=f'{self.ortho_loss_weight}'.split('.')[0] + f'{self.ortho_loss_weight}'.split('.')[1] if len(f'{self.ortho_loss_weight}'.split('.')) > 1 else f'{self.ortho_loss_weight}'
        # logging_dir='/home/u2022211776/NIPS_new/Output/Tensorboard/deepseek_new_data_3k/deepseek-moe-16b-chat-test/Test_a_'+aaa+'_v_'+vvv+'_o_'+ooo+f'/{timestamp}'      
    ...
    def compute_weight(self, hidden_states):
        """
        Compute the weight of the gating function based on the hidden states.
        Args:
            hidden_states: [batch_size, seq_len, hidden_dim]
        Returns:
            None
        """
        global oloss_w, vloss_w
        
        if oloss_w == -1 and vloss_w == -1:
            with torch.no_grad():
                # Compute orthogonality loss
                expert_outputs = self.weight.unsqueeze(0).expand(hidden_states.size(0), -1, -1)
                ortho_loss = self.compute_ortho_loss(expert_outputs)

                # Compute variance loss (consistent with forward)
                logits = F.linear(hidden_states.view(-1, hidden_states.size(-1)), self.weight, None)
                scores = logits.softmax(dim=-1)
                variance_loss = torch.var(scores, dim=-1).mean()
                variance_loss_weight = self.variance_loss_target / (variance_loss)

                if self.ortho_loss_weight != 0:
                    ortho_loss_weight = self.ortho_loss_target / (ortho_loss)
                    ortho_loss_weight = 10 ** torch.round(torch.log10(ortho_loss_weight))
                else:
                    ortho_loss_weight = 0

                if self.variance_loss_target != 0:
                    variance_loss_weight = self.variance_loss_target / (variance_loss)
                    variance_loss_weight = 10 ** torch.round(torch.log10(variance_loss_weight))
                else:
                    variance_loss_weight = 0

                # Update weights
                self.ortho_loss_weight = ortho_loss_weight
                self.variance_loss_weight = variance_loss_weight
                oloss_w = ortho_loss_weight
                vloss_w = variance_loss_weight

        else:
            # Use the pre-defined weights
            self.ortho_loss_weight = oloss_w
            self.variance_loss_weight = vloss_w


    def forward(self, hidden_states, step=None):
        global step_cnt
        bsz, seq_len, h = hidden_states.shape

        if self.variance_loss_weight == -1 and self.ortho_loss_weight == -1:
            self.compute_weight(hidden_states)
        ...
        # Orthogonality loss (will be computed in DeepseekMoE after getting expert outputs)
        ortho_loss = torch.tensor(0.0, device=hidden_states.device)

        # Combine all losses
        total_loss = aux_loss - variance_loss + ortho_loss
        ...
        if self.training and self.alpha > 0.0:
            with torch.no_grad():
                ...
                ideal_load = self.top_k * total_tokens / self.n_routed_experts  
                
                assigned_expert_idx = topk_idx.view(-1)  # shape: [total_tokens * top_k]
                load_count = torch.bincount(assigned_expert_idx, minlength=self.n_routed_experts).float()

                diff = torch.abs(load_count - ideal_load)
                max_diff = diff.max()
                maxvloglobal = (max_diff / ideal_load).item()

                if step_cnt % logging_steps == 0:
                    self.writer.add_scalar(f"Gating/MaxVloglobal/MoElayer_{self.layer_idx}", maxvloglobal, step_cnt)
    ...
    def forward(self, hidden_states):
        ...
        if self.training:
            ...
            if self.num_experts_per_tok > 1:
                # Reshape y to [total_tokens, top_k, hidden_dim]
                ...
                # Logging
                if hasattr(self.gate, 'writer') and self.gate.writer is not None:
